    ...
    <div class="max-w-2xl">
        <div class="card">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h2 class="text-lg font-bold">Add Homework</h2>
                    <p class="text-sm text-[rgb(var(--color-text-muted))]">Create new homework for {{ $course->title }}
                    </p>
                </div>
            </div>
            <form action="{{ route('dashboard.homeworks.store', $course->slug) }}" method="POST" class="space-y-5">
                ...
                <div><label for="title" class="form-label">Homework Title</label><input type="text" id="title"
                        name="title" value="{{ old('title') }}" class="form-input @error('title') is-invalid @enderror"
                ...
                <div><label for="content" class="form-label">Instructions</label><textarea id="content" name="content"
                        ...
                        placeholder="Enter homework instructions...">{{ old('content') }}</textarea>@error('content')<p
